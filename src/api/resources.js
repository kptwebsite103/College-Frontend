...
export function compressImage(
  ...
  return new Promise((resolve) => {
    ...
    if (file.size < 100 * 1024) {
      ...
      console.log(
        "📏 Small image detected, skipping compression:",
    ...
    img.onload = () => {
      ...
      if (width > maxWidth || height > maxHeight) {
        ...
        height *= ratio;
        console.log(
          `📐 Resizing image from ${originalWidth}x${originalHeight} to ${Math.round(width)}x${Math.round(height)}`,
        );
      } else {
        console.log(
          `📏 Image dimensions acceptable (${originalWidth}x${originalHeight}), no resizing needed`,
        );
      }
      ...
      canvas.toBlob(
        (blob) => {
          ...
          if (compressedFile.size < file.size) {
            ...
            console.log(
              `✅ Compression successful: ${formatFileSize(file.size)} → ${formatFileSize(compressedFile.size)} (${savings}% smaller)`,
            );
            resolve(compressedFile);
          } else {
            console.log(
              `⚠️ Compression would increase size, using original: ${formatFileSize(file.size)} → ${formatFileSize(compressedFile.size)}`,
            );
            resolve(file);
          }
        },
...
export function uploadFile(file, onProgress, options = {}) {
  ...
  if (file.size > maxFileSize) {
    return Promise.reject(
      new Error(`File size exceeds ${maxFileSize / (1024 * 1024)}MB limit`),
    );
  }
...
export function listMedia() {
  const ts = Date.now();
  return apiRequest(`/api/media?_t=${ts}`);
}
